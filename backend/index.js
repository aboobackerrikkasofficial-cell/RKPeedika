...
/*
|--------------------------------------------------------------------------
| Auto Seed Reviews on Startup
|--------------------------------------------------------------------------
*/
async function autoSeedReviews() {
  try {
    const productsWithoutReviews = await prismaDb.product.findMany({
      where: { reviewCount: 0 }
    });

    if (productsWithoutReviews.length === 0) {
      console.log('✅ No products need review seeding.');
      return;
    }

    console.log(`⏳ Auto-seeding reviews for ${productsWithoutReviews.length} products...`);
    ...
    for (const prod of productsWithoutReviews) {
      const targetReviewCount = Math.floor(Math.random() * (85 - 20 + 1)) + 20; 
      const manualBalance = Math.floor(Math.random() * 2) + 5; 
      const generatedReviewCount = targetReviewCount - manualBalance;
    ...
    console.log(`✅ Auto-seeding completed for ${productsWithoutReviews.length} products.`);
  } catch (error) {
    console.error('❌ Error in autoSeedReviews:', error);
  }
}
