  ...
  const inputCls = (field) =>
    `w-full rounded-xl border px-3.5 py-2.5 text-xs text-gray-800 outline-none transition-all ${
      errors[field]
        ...
        : 'border-gray-200 bg-white focus:border-[#F7941D] focus:ring-2 focus:ring-orange-100'
    }`;
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-4">
        ...
        <Field label="Pincode" icon={MapPin} error={errors.pincode}>
          <div className="relative">
            ...
            <span className="absolute right-3 top-1/2 -translate-y-1/2">
              {pincodeStatus === 'checking' && <Loader className="h-3.5 w-3.5 text-orange-400 animate-spin" />}
      ...
      {pincodeStatus === 'checking' && (
        <p className="flex items-center gap-1.5 text-[11px] font-semibold text-orange-400 -mt-1">
      ...
      <div className="flex items-center gap-3 pt-2">
        <button
          type="submit"
          disabled={saving}
          className="flex-1 md:flex-none rounded-xl bg-[#F7941D] px-8 py-3 text-xs font-bold text-white hover:bg-[#E07D10] transition-all shadow shadow-orange-500/10 disabled:opacity-60 disabled:cursor-not-allowed min-h-[44px]"
        >
