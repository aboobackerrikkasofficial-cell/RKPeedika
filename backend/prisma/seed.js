...
async function main() {
  ...
  if (isDemoMode) {
    ...
    for (const prod of products) {
      // Logic for random target review count and manual balance
      ...
      for (let i = 0; i < generatedReviewCount; i++) {
        ...
        // Keep ratings realistic (mostly 4 and 5, occasional 3)
      ...
      await prisma.product.update({
        where: { id: prod.id },
        data: { 
          reviewCount: generatedReviewCount, // Let it update naturally as they add manual ones
          rating: Number(avgRating.toFixed(1)),
          averageRating: Number(avgRating.toFixed(1))
        }
      });
      console.log(`✔ Generated ${generatedReviewCount} automated reviews for ${prod.name} (reserved ${manualBalance} for manual entry)`);
    }
  }
